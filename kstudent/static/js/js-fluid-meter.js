...
function FluidMeter() {
  ...
  function drawFluidLayer(layer, dt) {
    ...
    // Change the percentage formula

    var fluidAmount = currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / 100;

    // end Change the percentage formula 
  ...
  function drawText() {
    ...
    var text = options.drawPercentageSign ?
      currentFillPercentage.toFixed(0) + "%" : (currentFillPercentage * totalMarks / 100).toFixed(0) + "/" + parseInt(totalMarks);
  ...
  return {
    init: function (env) {
      ...
      totalMarks = clamp(env.totalMarks, 0, 100);
